...
function getDeckPrompt(accountId) {
  var accounts = cachedAccounts_();
  if (!accounts.length) return '';
  // Scope to the requested (dashboard-current) account, not just the first
  // cached one, so the bundle prompt matches the deck being built.
  var id = '';
  if (accountId) {
    var want = digits_(accountId);
    accounts.some(function(a) {
      if (digits_(a.id) === want) { id = a.id; return true; }
      return false;
    });
  }
  if (!id) id = accounts[0].id;
...
/**
 * The AI analysis as a standalone, human-readable markdown doc for the bundle.
 * `accountId` scopes it to the dashboard-current account (falls back to the
 * first cached one). Returns '' when no analysis has been run.
 */
function aiMarkdown_(accountId) {
  var id = digits_(accountId || '');
  if (!id) {
    var accs = cachedAccounts_();
    id = accs.length ? digits_(accs[0].id) : '';
  }
  if (!id) return '';
  var a = null;
  try { a = readAi_(id); } catch (e) { a = null; }
  if (!a) return '';

  var out = [];
  out.push('# Lockhern AI strategist analysis');
  out.push('');
  out.push('Generated by ' + (a._model || 'Claude') + ' as a ' +
      (a._mode === 'strategy' ? 'Demand Gen launch' : 'Demand Gen audit') +
      ' analysis of this account. This is the strategist read the branded deck ' +
      'leads with; it is also embedded in PROMPT.md as authoritative direction.');
  out.push('');
  if ((a.executiveSummary || []).length) {
    out.push('## Executive summary');
    a.executiveSummary.forEach(function(f) {
      out.push('- **[' + f.tag + '] ' + f.stat + '** — ' + f.label + '. ' +
          f.detail);
    });
    out.push('');
  }
  if (a.oneThing) {
    out.push('## The one thing that matters most');
    out.push(a.oneThing);
    out.push('');
  }
  if ((a.recommendations || []).length) {
    ...
    a.recommendations.forEach(function(r) {
      out.push('### ' + r.title + '  _(' + r.area + ' · ' + r.priority + ')_');
      out.push(r.insight);
      out.push('');
      out.push('**Do:** ' + r.action);
      out.push('');
    });
  }
  if (a.designDirection) {
    out.push('## Design direction');
    out.push(a.designDirection);
    out.push('');
  }
  return out.join('\n');
}
...
function bundleReadme_() {
  return [
    ...
    '- AI-ANALYSIS.md       if present, the Lockhern AI strategist read the deck',
    '                       leads with (also embedded inside PROMPT.md)',
 ...
 */
function buildBundle(deckUrls, screenshots, uiLogos, accountId) {
  try {
    var token = ScriptApp.getOAuthToken();
    var files = [];
    var included = [];
    ...
    add(Utilities.newBlob(getDeckPrompt(accountId), 'text/markdown', 'PROMPT.md'));
    // The Lockhern AI strategist analysis as its own readable doc, so it travels
    // with the bundle (it is also embedded inside PROMPT.md as authoritative
    // direction). Skipped silently when the analysis has not been run.
    var aiMd = aiMarkdown_(accountId);
    if (aiMd) {
      add(Utilities.newBlob(aiMd, 'text/markdown', 'AI-ANALYSIS.md'));
    } else {
      warnings.push('No AI analysis yet — run "Analyze with AI" first to include ' +
          'AI-ANALYSIS.md and lead the deck with it.');
    }
